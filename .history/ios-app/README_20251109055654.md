# 📱 CirurgiaoApp - iOS

Aplicativo iOS nativo do Projeto Cirurgião desenvolvido em Swift 5.9 + SwiftUI.

## 📋 Índice

- [Visão Geral](#visão-geral)
- [Requisitos](#requisitos)
- [Instalação](#instalação)
- [Arquitetura](#arquitetura)
- [Estrutura do Projeto](#estrutura-do-projeto)
- [Funcionalidades](#funcionalidades)
- [Testes](#testes)
- [Configuração](#configuração)
- [API Integration](#api-integration)

## 🎯 Visão Geral

Aplicativo iOS para o sistema de gestão cirúrgica, implementando autenticação completa e dashboard inicial.

### Stack Tecnológica

- **Swift**: 5.9
- **UI Framework**: SwiftUI
- **Arquitetura**: MVVM (Model-View-ViewModel)
- **Async**: async/await + Combine
- **Networking**: URLSession
- **Storage**: Keychain
- **Analytics**: Firebase Analytics
- **Crash Reporting**: Firebase Crashlytics
- **Minimum iOS**: 16.0

## 📦 Requisitos

- Xcode 15.0+
- iOS 16.0+
- macOS 13.0+ (para desenvolvimento)
- Swift 5.9+
- CocoaPods ou Swift Package Manager

## 🚀 Instalação

### 1. Clone o Repositório

```bash
cd ios-app
```

### 2. Instalar Dependências

O projeto usa Swift Package Manager. As dependências serão baixadas automaticamente ao abrir o projeto no Xcode.

**Dependências:**
- Firebase iOS SDK (Analytics, Crashlytics, Auth)

### 3. Configurar Firebase

O arquivo `GoogleService-Info.plist` já está incluído em:
```
ios-app/CirurgiaoApp/Resources/GoogleService-Info.plist
```

### 4. Abrir no Xcode

```bash
open CirurgiaoApp.xcodeproj
```

Ou use o Package.swift:
```bash
xed .
```

### 5. Configurar Backend URL

Por padrão, o app aponta para `http://localhost:3000`. Para alterar:

Edite `ios-app/CirurgiaoApp/Core/Network/APIClient.swift`:

```swift
private init() {
    self.baseURL = "http://localhost:3000" // Altere aqui
    // ...
}
```

### 6. Build e Run

1. Selecione um simulador ou dispositivo
2. Pressione `Cmd + R` ou clique em "Run"

## 🏗️ Arquitetura

### MVVM Pattern

```
┌─────────────┐
│    View     │ ← SwiftUI Views
└──────┬──────┘
       │
       ↓
┌─────────────┐
│  ViewModel  │ ← Business Logic + State
└──────┬──────┘
       │
       ↓
┌─────────────┐
│    Model    │ ← Data Models
└─────────────┘
       │
       ↓
┌─────────────┐
│  Services   │ ← API, Storage, etc.
└─────────────┘
```

### Camadas

1. **Views**: Interface do usuário (SwiftUI)
2. **ViewModels**: Lógica de apresentação e estado
3. **Models**: Estruturas de dados
4. **Services**: Serviços (API, Keychain, etc.)

## 📁 Estrutura do Projeto

```
ios-app/
├── CirurgiaoApp/
│   ├── App/
│   │   ├── CirurgiaoAppApp.swift      # Entry point
│   │   └── ContentView.swift          # Root view
│   │
│   ├── Features/
│   │   ├── Auth/
│   │   │   ├── Models/
│   │   │   │   └── User.swift         # User model
│   │   │   ├── ViewModels/
│   │   │   │   └── AuthViewModel.swift
│   │   │   └── Views/
│   │   │       ├── LoginView.swift
│   │   │       ├── RegisterView.swift
│   │   │       └── ForgotPasswordView.swift
│   │   │
│   │   └── Dashboard/
│   │       └── Views/
│   │           └── DashboardView.swift
│   │
│   ├── Core/
│   │   ├── Network/
│   │   │   └── APIClient.swift        # HTTP client
│   │   └── Storage/
│   │       └── KeychainManager.swift  # Secure storage
│   │
│   └── Resources/
│       ├── Info.plist
│       └── GoogleService-Info.plist
│
├── CirurgiaoAppTests/
│   └── AuthViewModelTests.swift       # Unit tests
│
├── Package.swift                       # SPM dependencies
└── README.md
```

## ✨ Funcionalidades

### ✅ Implementadas (Semana 2)

#### Autenticação
- [x] Login com email/senha
- [x] Registro de novos usuários
- [x] Recuperação de senha
- [x] Logout
- [x] Persistência de sessão (Keychain)
- [x] Refresh token automático

#### Dashboard
- [x] Tela inicial pós-login
- [x] Exibição de informações do usuário
- [x] Cards de funcionalidades
- [x] Navegação condicional

#### Segurança
- [x] Tokens salvos no Keychain
- [x] HTTPS/HTTP configurável
- [x] Validação de formulários

### 🔜 Próximas Funcionalidades (Semana 3+)

- [ ] Agendamentos
- [ ] Lista de pacientes
- [ ] Perfil do usuário
- [ ] Notificações push
- [ ] Modo offline
- [ ] Biometria (Face ID / Touch ID)

## 🧪 Testes

### Executar Testes

```bash
# Via Xcode
Cmd + U

# Via linha de comando
xcodebuild test -scheme CirurgiaoApp -destination 'platform=iOS Simulator,name=iPhone 15'
```

### Cobertura de Testes

- ✅ AuthViewModel
- ✅ KeychainManager
- ✅ User Model
- ✅ API Client (parcial)

### Testes Implementados

```swift
// AuthViewModel
- testInitialState()
- testLoginWithEmptyCredentials()
- testLogout()

// KeychainManager
- testSaveAndRetrieveAccessToken()
- testDeleteAllTokens()
- testOverwriteToken()

// User Model
- testUserDecoding()
- testUserRoleDecoding()
```

## ⚙️ Configuração

### Info.plist

Configurações importantes:

```xml
<!-- Permitir HTTP para localhost -->
<key>NSAppTransportSecurity</key>
<dict>
    <key>NSAllowsArbitraryLoads</key>
    <true/>
</dict>

<!-- Bundle ID -->
<key>CFBundleIdentifier</key>
<string>com.projeto-cirurgiao.app</string>
```

### Firebase

Configurado automaticamente no `CirurgiaoAppApp.swift`:

```swift
init() {
    FirebaseApp.configure()
}
```

## 🔌 API Integration

### Endpoints Utilizados

```swift
// Autenticação
POST /auth/login
POST /auth/register
POST /auth/refresh
GET  /auth/me
POST /auth/logout
```

### Exemplo de Uso

```swift
// Login
await authViewModel.login(
    email: "user@example.com",
    password: "password123"
)

// Registro
await authViewModel.register(
    email: "new@example.com",
    password: "password123",
    name: "Nome Completo",
    role: .patient
)

// Logout
await authViewModel.logout()
```

### Headers

```
Content-Type: application/json
Authorization: Bearer {access_token}  // Para rotas protegidas
```

## 🔐 Segurança

### Keychain

Tokens são armazenados de forma segura:

```swift
// Salvar
try KeychainManager.shared.saveAccessToken(token)

// Recuperar
let token = KeychainManager.shared.getAccessToken()

// Deletar
try KeychainManager.shared.deleteAllTokens()
```

### Validações

- Email: formato válido
- Senha: mínimo 6 caracteres
- Campos obrigatórios
- Confirmação de senha

## 📱 Navegação

### Fluxo de Autenticação

```
ContentView
    ├─ isAuthenticated = false → LoginView
    │                              ├─ RegisterView
    │                              └─ ForgotPasswordView
    │
    └─ isAuthenticated = true  → DashboardView
```

### Navegação Condicional

```swift
@EnvironmentObject var authViewModel: AuthViewModel

if authViewModel.isAuthenticated {
    DashboardView()
} else {
    LoginView()
}
```

## 🐛 Debug

### Logs

O app usa `print()` para logs de desenvolvimento:

```swift
print("✅ Login realizado com sucesso")
print("❌ Erro no login: \(error)")
print("📧 Email de recuperação enviado")
```

### Breakpoints

Pontos importantes para debug:
- `AuthViewModel.login()`
- `APIClient.request()`
- `KeychainManager.save()`

## 📊 Performance

### Otimizações

- ✅ Async/await para operações assíncronas
- ✅ @MainActor para UI updates
- ✅ Lazy loading de views
- ✅ Reuso de componentes

## 🤝 Contribuindo

### Code Style

- Swift Style Guide da Apple
- SwiftLint (configurar se necessário)
- Comentários em português
- MARK para organização

### Commits

```bash
git commit -m "feat: adiciona tela de perfil"
git commit -m "fix: corrige bug no login"
git commit -m "test: adiciona testes para ProfileViewModel"
```

## 📝 Notas da Semana 2

### Implementado por: Lucas (IOS-SENIOR-01)

**Entregas:**
- ✅ Setup do projeto iOS
- ✅ Arquitetura MVVM
- ✅ Telas de autenticação
- ✅ Integração com backend
- ✅ Keychain para tokens
- ✅ Firebase configurado
- ✅ Testes unitários
- ✅ Documentação

**Próximos Passos:**
1. Implementar funcionalidades do dashboard
2. Adicionar biometria
3. Implementar notificações push
4. Modo offline com Core Data
5. Melhorar cobertura de testes

## 📞 Suporte

Para dúvidas ou problemas:
- Consulte a documentação do projeto
- Verifique os logs do Xcode
- Revise os testes unitários

## 📄 Licença

Projeto Cirurgião - Propriedade privada
