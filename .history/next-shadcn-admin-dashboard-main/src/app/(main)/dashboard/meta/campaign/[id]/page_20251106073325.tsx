...
import { getMetaCampaigns, getCampaignInsights } from "../../_actions/meta-actions";
...
export default async function CampaignDetailsPage({ params }: PageProps) {
  ...
  const campaign = campaignsData.campaigns.find((c) => c.id === params.id);
  ...
  const insightsData = await getCampaignInsights(campaign.id, "last_30d");
    ...
    <div className="@container/main flex flex-col gap-4 md:gap-6">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="audience" className="mt-4">
          <CampaignAudienceTab campaign={campaign} insights={insights} />
        </TabsContent>
      </Tabs>
    </div>
  );
