...
export default async function MetaPage() {
  ...
  const activeCampaignsWithInsights = campaignsWithInsights.filter((c) => c.status === "ACTIVE");
  const pausedCampaignsWithInsights = campaignsWithInsights.filter((c) => c.status === "PAUSED");
    ...
    <div className="@container/main flex flex-col gap-4 md:gap-6">
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="all" className="mt-4">
          <MetaCampaignsTable campaigns={campaignsWithInsights} />
        </TabsContent>
        <TabsContent value="active" className="mt-4">
          <MetaCampaignsTable campaigns={activeCampaignsWithInsights} />
        </TabsContent>
        <TabsContent value="paused" className="mt-4">
          <MetaCampaignsTable campaigns={pausedCampaignsWithInsights} />
        </TabsContent>
      </Tabs>

      {activeCampaignsWithInsights.length > 0 && (
        <div className="space-y-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight">
              Detalhes da Campanha em Destaque
            </h2>
            <p className="text-muted-foreground">
              Análise detalhada da campanha com melhor desempenho
            </p>
          </div>
          <CampaignDetailsCard campaign={activeCampaignsWithInsights[0]} />
        </div>
      )}
    </div>
  );
