...
export async function GET(
  ...
  try {
    ...
    // Buscar insights da campanha com breakdown diário usando o MCP server
    const mcpResponse = await fetch("http://localhost:3000/api/mcp", {
      ...
      body: JSON.stringify({
        ...
        tool: "get_campaign_insights_breakdown",
        arguments: {
    ...
    const breakdown = mcpData.breakdown || [];
    
    // Transformar os insights em dados diários
    const dailyInsights = breakdown.map((insight: any) => {
